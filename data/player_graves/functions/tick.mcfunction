...
# Drop player's head if killed by player
execute as @a[scores={pg_death=1..},limit=1] at @s if score @s killedByPlayer matches 1.. run loot spawn ~ ~2 ~ loot player_graves:head
execute if score @a[scores={pg_death=1..},limit=1] killedByPlayer matches 1.. run scoreboard players reset @a killedByPlayer
